...
**`nrighar-api` ALSO needs `TEMPORAL_ADDRESS` (found 2026-07-24).** Since PR #8 routed every endpoint through Temporal, the api is a Temporal *client* — it calls `client.workflow.execute()`, which needs to reach the same server. `api/src/temporal/env.ts` defaults to `localhost:7233` when unset, so a missing `TEMPORAL_ADDRESS` means the api tries to connect inside its own container, times out (~10s), and every request 500s — this is exactly what broke login after the migration. Set `TEMPORAL_ADDRESS` on the `nrighar-api` resource to the **same value as the worker** (`<temporal container's current name>:7233`), and keep them in sync whenever the `temporal` container name changes. `TEMPORAL_NAMESPACE`/`TEMPORAL_TASK_QUEUE` default correctly in code (`default`/`nrighar`), so only `TEMPORAL_ADDRESS` is strictly required on the api.

**Migrations run at container start (changed 2026-07-24).** `api/Dockerfile`'s `CMD` now runs `node dist/db/migrate.js && node dist/index.js`, so pending Drizzle migrations apply automatically on every `nrighar-api` deploy — no Coolify pre-deploy command needed. The old `npm run db:migrate` (`tsx src/db/migrate.ts`) could never run in the `--omit=dev` runtime image (no `tsx`, no `src/`), so it silently never applied migrations and schema drift reached prod (a missing `bedrooms` column from PR #12 500'd the properties page). Use `npm run db:migrate:prod` (compiled) for any manual run inside the container; `npm run db:migrate` is dev/local-only.

**Temporal schedules are registered by the worker at boot (added 2026-07-26).** `api/src/temporal/schedules.ts` runs before `worker.run()` and `create()`s every entry in its `SCHEDULES` list, swallowing `ScheduleAlreadyRunning` — so a redeploy is a no-op and no manual `temporal schedule create` step exists. Two consequences: (1) changing a cron expression in that file does **not** update a schedule that already exists on the server — `temporal schedule delete --schedule-id <id>` and redeploy, or edit it in the Temporal UI; (2) schedule registration failures are logged, never thrown, because the worker must still poll the queue that serves every HTTP route. Verify with `temporal schedule list` (or the Schedules tab in the Temporal UI) after a deploy. Currently one schedule: `nrighar-purge-expired-sessions`, daily 03:30 UTC, hard-deletes `sessions` rows past their 30-day expiry.
